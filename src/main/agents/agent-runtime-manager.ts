...
- Browser inspection follows the Agent Browser contract: use public tab handles such as \`t1\` and short element refs such as \`@e1\`; never invent refs, selectors, coordinates, or internal tab ids. Re-inspect after actions because refs are fresh per snapshot. Link URLs are omitted by default and should be requested only when needed.
