...
async function ensureProviderColumns(db: AppDatabase): Promise<void> {
  ...
  await addColumn(db, 'app_providers', 'supports_thinking', 'INTEGER NOT NULL DEFAULT 1')
  await addColumn(db, 'app_providers', 'thinking_only', 'INTEGER NOT NULL DEFAULT 0')
  await addColumn(db, 'app_providers', 'allows_thinking_off', 'INTEGER NOT NULL DEFAULT 1')
  await addColumn(db, 'app_providers', 'default_thinking_level', "TEXT NOT NULL DEFAULT 'medium'")
  await addColumn(db, 'app_providers', 'supported_thinking_levels', "TEXT NOT NULL DEFAULT '[]'")
