...
目前，TIA Studio 已支持 Lark channels。Telegram 和 WhatsApp 支持也会很快到来，后续还会继续扩展更多 channels。
...
1. **先创建 assistant** - `POST /v1/claws` 会先创建一个普通 assistant，然后要么新建一个 Lark channel，要么把一个尚未绑定的已有 channel 挂到这个 assistant 上
...
- ✅ 现在已支持 Lark
- 🚧 Telegram 和 WhatsApp 即将支持，后续还有更多 channels
- 💬 基于 threads 的会话管理
- 🎨 干净、深色主题的界面
- 🔒 Local-first 架构
- ⚡ 快速、原生的桌面性能
