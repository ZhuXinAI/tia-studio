...
目前，TIA Studio 已支持 Discord、Lark、Telegram、WhatsApp、企业微信和微信客服 channels，后续还会继续扩展更多 channels。

### Channel 支持情况

| Channel | 私聊 | 群聊 | 群聊触发方式 |
| ------- | ---- | ---- | ------------ |
| Discord | ✅ | ✅ | 默认只有在 `@` 提及 bot 时才会回复 |
| Lark | ✅ | ✅ | 默认只有在 `@` 提及 bot 时才会回复 |
| Telegram | ✅ | 🚫 | 当前暂不支持群聊 |
| WhatsApp | ✅ | ✅ | 默认只有在 `@` 提及 bot 时才会回复 |
| 企业微信 | ✅ | ✅ | 默认只有在 `@` 提及 bot 时才会回复 |
| 微信客服 | ✅ | 🚫 | 不适用，当前仅支持 relay 接入的一对一客服会话 |
...
- ✅ 现在已支持 Discord、Lark、Telegram、WhatsApp、企业微信和微信客服
- 🚧 后续还有更多 channels
- 💬 基于 threads 的会话管理
- 🎨 干净、深色主题的界面
- 🔒 Local-first 架构
